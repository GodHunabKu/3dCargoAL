...
function hg_lib.is_vnum_in_emerg_list(vnum)
    -- Controlla se un vnum   tra i vnums dell'emergency
...
-- ============================================================
-- PITY SYSTEM: Dopo X sconfitte consecutive, difesa piu' facile
-- ============================================================

-- Configurazione Pity System
local PITY_THRESHOLD = 5      -- Sconfitte consecutive per attivare pity
local PITY_REDUCTION = 0.50   -- Riduzione mob (50% = meta' mob)
local PITY_DURATION_BONUS = 30 -- Secondi extra di tempo

-- Controlla se il Pity System e' attivo per il player
function hg_lib.is_pity_active(pid)
    pid = pid or pc.get_player_id()
    local fail_streak = pc.getqf("hq_defense_fail_streak") or 0
    return fail_streak >= PITY_THRESHOLD
end

-- Ottiene lo streak di sconfitte corrente
function hg_lib.get_defense_fail_streak(pid)
    pid = pid or pc.get_player_id()
    return pc.getqf("hq_defense_fail_streak") or 0
end

-- Incrementa il contatore sconfitte (chiamato da fail_defense)
function hg_lib.increment_defense_fail_streak(pid)
    pid = pid or pc.get_player_id()
    local current = pc.getqf("hq_defense_fail_streak") or 0
    current = current + 1
    pc.setqf("hq_defense_fail_streak", current)
    
    -- Notifica se sta per attivare pity
    if current == PITY_THRESHOLD - 1 then
        syschat("|cffFFFF00[SISTEMA]|r Ancora una sconfitta e il Sistema ti aiutera'...")
    elseif current == PITY_THRESHOLD then
        syschat("|cff00FF00============================================|r")
        syschat("|cff00FF00[PITY SYSTEM ATTIVATO!]|r")
        syschat("|cff00FF00Il Sistema ha notato le tue difficolta'.|r")
        syschat("|cffFFFF00La prossima difesa sara' PIU' FACILE!|r")
        syschat("|cffFFFF00- Mob ridotti del 50%|r")
        syschat("|cffFFFF00- Tempo extra +30 secondi|r")
        syschat("|cff00FF00============================================|r")
        hg_lib.hunter_speak_color("PITY ATTIVO! La prossima difesa sara' facilitata!", "GREEN")
    end
    
    return current
end

-- Resetta il contatore (chiamato da complete_defense_success)
function hg_lib.reset_defense_fail_streak(pid)
    pid = pid or pc.get_player_id()
    local was_pity = hg_lib.is_pity_active(pid)
    pc.setqf("hq_defense_fail_streak", 0)
    
    if was_pity then
        syschat("|cff00FF00[SISTEMA]|r Pity System disattivato. Ottimo lavoro, Hunter!")
    end
end

-- Applica la riduzione Pity ai mob richiesti
function hg_lib.apply_pity_reduction(total_mobs, pid)
    pid = pid or pc.get_player_id()
    
    if hg_lib.is_pity_active(pid) then
        local reduced = math.floor(total_mobs * PITY_REDUCTION)
        -- Minimo 3 mob per non essere troppo facile
        if reduced < 3 then reduced = 3 end
        return reduced, true
    end
    
    return total_mobs, false
end

-- Applica il bonus tempo Pity
function hg_lib.apply_pity_duration_bonus(duration, pid)
    pid = pid or pc.get_player_id()
    
    if hg_lib.is_pity_active(pid) then
        return duration + PITY_DURATION_BONUS, true
    end
...
-- ============================================================
-- FIRST OF DAY BONUS SYSTEM
-- Bonus Gloria per la prima attivita' del giorno
-- Incentiva il login giornaliero con ricompense extra
-- ============================================================

-- Resetta tutti i flag "First of Day" (chiamato al reset giornaliero o login)
-- ============================================================
-- FIRST OF DAY SYSTEM - Usa game.set_event_flag per persistenza
-- ============================================================
...
function hg_lib.reset_first_of_day_flags()
    ...
    if last_fod_date ~= today then
        ...
        pc.setqf("hq_fod_boss", 0)      -- Primo boss del giorno
        pc.setqf("hq_fod_chest", 0)     -- Primo baule del giorno
        pc.setqf("hq_fod_metin", 0)     -- Primo metin del giorno
        -- Reset anche game event flags (persistenza immediata)
...
-- Restituisce: is_first, bonus_multiplier
function hg_lib.check_first_of_day(activity_type)
    local pid = pc.get_player_id()
    
    -- Assicurati che i flag siano aggiornati per oggi
    hg_lib.reset_first_of_day_flags()
    
    local flag_name = "hq_fod_" .. activity_type
    local config_key = "first_" .. activity_type .. "_bonus"
    local event_flag_name = "hq_fod_done_" .. activity_type .. "_" .. pid
    
    -- CHECK 1: Event flag (persistenza immediata durante sessione)
    local event_done = game.get_event_flag(event_flag_name) or 0
    if event_done == 1 then
        return false, 1.0  -- Gia' fatto in questa sessione
    end
    
    -- CHECK 2: Quest flag (persistenza DB)
    local already_done = tonumber(pc.getqf(flag_name)) or 0
    if already_done == 1 then
        -- Sincronizza event flag con quest flag
        game.set_event_flag(event_flag_name, 1)
        return false, 1.0  -- Gia' fatto, nessun bonus
    end
    
    -- E' la prima volta! Recupera il bonus dalla config
    local bonus_percent = tonumber(hg_lib.get_config(config_key)) or 0
    if bonus_percent <= 0 then
        -- FIX: Marca come fatto ANCHE se bonus non configurato (evita spam)
        pc.setqf(flag_name, 1)
        game.set_event_flag(event_flag_name, 1)
        return false, 1.0  -- Bonus non configurato
    end
    
    -- Marca come fatto per oggi (sia quest flag che event flag)
    pc.setqf(flag_name, 1)
    game.set_event_flag(event_flag_name, 1)
    
    -- Calcola moltiplicatore (es: 50% bonus = 1.5x)
    local multiplier = 1.0 + (bonus_percent / 100)
    
    return true, multiplier
end

-- Applica il bonus First of Day alla Gloria e mostra notifica
-- Restituisce: gloria finale, flag se bonus applicato
function hg_lib.apply_first_of_day_bonus(base_glory, activity_type, activity_name)
    local is_first, multiplier = hg_lib.check_first_of_day(activity_type)
    
    if not is_first then
        return base_glory, false
    end
    
    -- Calcola bonus
    local bonus_glory = math.floor(base_glory * (multiplier - 1.0))
    local final_glory = base_glory + bonus_glory
    
    -- Notifica speciale per il primo del giorno
    activity_name = activity_name or activity_type
    local bonus_percent = math.floor((multiplier - 1.0) * 100)
    
    syschat("|cff00FF00============================================|r")
    syschat("|cffFFD700*** PRIMO " .. string.upper(activity_name) .. " DEL GIORNO! ***|r")
    syschat("|cff00FF00Bonus Giornaliero: +|r|cffFFD700" .. bonus_percent .. "%|r |cff00FF00(+" .. bonus_glory .. " Gloria)|r")
    syschat("|cff00FF00============================================|r")
    
    -- Effetto client
    cmdchat("HunterFirstOfDay " .. activity_type .. "|" .. bonus_glory .. "|" .. bonus_percent)
    
    return final_glory, true
end

-- ============================================================
-- FINE FIRST OF DAY BONUS SYSTEM
-- ============================================================
...
function hg_lib.flush_ranking_updates()
    ...
    -- Check se c'  qualcosa da flushare (base o detailed)
    ...
    if has_pending or has_detailed then
        ...
        -- KARMA: Diviso 10 perche' il karma interno e' 10x quello visualizzato
        if pending_total_pts > 0 then
            pc.change_alignment(math.floor(pending_total_pts / 10))
        end
...
function hg_lib.flush_trial_progress()
    ...
    local q = "UPDATE srv1_hunabku.hunter_player_trials SET boss_kills = boss_kills + " .. boss_kills .. ", metin_kills = metin_kills + " .. metin_kills .. ", chest_opens = chest_opens + " .. chest_opens .. ", fracture_seals = fracture_seals + " .. fracture_seals .. " WHERE player_id = " .. pid .. " AND status = 'in_progress'"
...
function hg_lib.is_dungeon_map(map_index)
    ...
    -- CH99 (dungeon channel): map_index >= 990000
    if map_index >= 990000 then
        return true
    end
...
-- Resetta le posizioni delle finestre UI sul client
function hg_lib.reset_ui_positions()
    cmdchat("HunterResetUI")
    hg_lib.hunter_speak_color("Le posizioni delle finestre verranno resettate. Riavvia il client per applicare le modifiche.", "BLUE")
end
...
-- SISTEMA EMERGENZE
-- Parametri opzionali: description, difficulty, penalty_pts, vnums (stringa con pi  vnum separati da virgola)
function hg_lib.start_emergency(title, seconds, mob_vnum, count, description, difficulty, penalty_pts, vnums)
    -- Non sovrascrivere una Emergency gi  attiva
    ...
    -- Supporto multi-vnum: se vnums   fornito, usa quello, altrimenti usa mob_vnum singolo
...
function hg_lib.end_emergency(status)
    ...
    if status == "SUCCESS" then
        if bonus_pts > 0 then
            ...
            -- KARMA: /10
            pc.change_alignment(math.floor(bonus_pts / 10))
        ...
        if penalty_pts > 0 then
            ...
            -- KARMA: /10 (negativo)
            pc.change_alignment(-math.floor(penalty_pts / 10))
...
function hg_lib.notify_rival(name, points, label)
    ...
    -- Se il cooldown non   scaduto, non inviare notifica
...
function hg_lib.load_config_cache()
    -- Ricarica solo se la cache   vuota o pi  vecchia di 10 minuti
    local now = get_time()
...
function hg_lib.calculate_glory_with_modifiers(base_glory, options)
    ...
    local activity_type = options.activity_type or nil  -- NEW: Per First of Day bonus

    local final_glory = base_glory
    local modifier_log = {}

    -- 0. FIRST OF DAY BONUS (se attivita' specificata)
    if activity_type then
        local is_first, fod_mult = hg_lib.check_first_of_day(activity_type)
        if is_first and fod_mult > 1.0 then
            local fod_add = math.floor(final_glory * (fod_mult - 1.0))
            final_glory = final_glory + fod_add
            local fod_percent = math.floor((fod_mult - 1.0) * 100)
            table.insert(modifier_log, {name = "Primo del Giorno!", value = "+" .. fod_percent .. "%", add = fod_add})
            -- Effetto client per primo del giorno
            cmdchat("HunterFirstOfDay " .. activity_type .. "|" .. fod_add .. "|" .. fod_percent)
        end
    end
...
function hg_lib.award_glory_to_player(player_id, glory_amount)
    ...
    -- KARMA INTEGRATION: Assegna karma uguale alla gloria (1:1)
    if glory_amount > 0 then
        local current_pid = pc.get_player_id()
        if current_pid == player_id then
            pc.change_alignment(math.floor(glory_amount / 10))
        end
    end
end
...
function hg_lib.distribute_party_glory_elite(base_glory, mob_info)
    ...
    -- Determina activity_type per First of Day in base al tipo di mob
    local activity_type = nil
    local type_name = (mob_info and mob_info.type_name) or "ELITE"
    if type_name == "BOSS" or type_name == "BOSS_ELITE" then
        activity_type = "boss"
    elseif type_name == "METIN" or type_name == "SUPER_METIN" then
        activity_type = "metin"
    elseif type_name == "BAULE" then
        activity_type = "chest"
    end

    -- USA FUNZIONE CENTRALIZZATA per calcolo modificatori (con First of Day)
    local final_glory, modifier_log = hg_lib.calculate_glory_with_modifiers(base_glory, {
        player_id = pid,
...
function hg_lib.draw_event_winner(event_id)
    ...
    if c > 0 then
        ...
        -- KARMA 1:1 - Salva nel DB per assegnarlo al login se offline
        mysql_direct_query(string.format(
            "UPDATE srv1_hunabku.hunter_quest_ranking SET pending_karma = IFNULL(pending_karma,0) + %d WHERE player_id=%d",
            glory_prize, winner_id
        ))
...
function hg_lib.check_event_end_and_draw()
    ...
    if c > 0 then
        for i = 1, c do
            ...
            -- FIX: Finestra estesa a 5 minuti per garantire che l'estrazione avvenga
            -- Se siamo al minuto di fine evento (finestra di 5 minuti per sicurezza)
            if current_total >= end_total and current_total < end_total + 5 then
                -- FIX RACE CONDITION: Usa event_flag come lock per evitare multiple esecuzioni
                local lock_flag = "hq_event_draw_lock_" .. event_id
                local lock_time_flag = "hq_event_draw_time_" .. event_id
                local is_locked = game.get_event_flag(lock_flag) or 0
                local lock_time = game.get_event_flag(lock_time_flag) or 0
                
                -- FIX: Flag per saltare questo evento se gia' in elaborazione
                local skip_event = false

                -- FIX: Se il lock esiste ma e' vecchio (>5 min), ignoralo (player andato offline)
                if is_locked == 1 then
                    if lock_time > 0 and (get_time() - lock_time) > 300 then
                        -- Lock stale, rilascialo
                        game.set_event_flag(lock_flag, 0)
                        is_locked = 0
                    else
                        -- Lock valido, salta questo evento ma continua con gli altri
                        skip_event = true
                    end
                end
                
                if not skip_event then
                    -- Acquisisci il lock con timestamp
                    game.set_event_flag(lock_flag, 1)
                    game.set_event_flag(lock_time_flag, get_time())
...
-- Se il player ha un'emergency quest attiva che non   scaduta,
...
function hg_lib.restore_emergency_on_login()
    ...
    if expire_time <= now then
        -- L'emergency   scaduta durante il logout - FALLIMENTO
...
function hg_lib.check_gate_selection()
    ...
    -- Cerca se il player ha un accesso Gate/Supremo pendente (FIX: rimosso granted_at e hunter_gate_config)
    local q = "SELECT ga.access_id, ga.gate_id, ga.expires_at, TIMESTAMPDIFF(MINUTE, NOW(), ga.expires_at) as minutes_left FROM srv1_hunabku.hunter_gate_access ga WHERE ga.player_id = " .. pid .. " AND ga.status = 'pending' AND ga.expires_at > NOW() ORDER BY ga.expires_at DESC LIMIT 1"

    local c, d = mysql_direct_query(q)

    if c > 0 and d[1] then
        -- Legge rank del giocatore per ottenere nome Supremo
        local rank = hg_lib.get_player_rank_grade(pid) or "E"
        local supremo_name = hg_lib.get_supremo_name_by_rank(rank) or "Supremo"
        ...
        cmdchat("HunterGateSelected " .. hg_lib.clean_str(supremo_name) .. "|" .. rank .. "|" .. hours_left .. "|" .. mins_left)
        ...
        syschat("|cffFFD700========================================|r")
        syschat("")
        hg_lib.syschat_t("GATE_SELECTED_MSG", "Puoi evocare: {GATE}", {GATE = supremo_name}, "00FF00")
        hg_lib.syschat_t("GATE_SELECTED_RANK", "Grado Supremo: {RANK}-Rank", {RANK = rank}, "00FFFF")
        hg_lib.syschat_t("GATE_SELECTED_TIME", "Tempo rimasto: {H}h {M}m", {H = hours_left, M = mins_left}, "FF6600")
        syschat("")
        hg_lib.syschat_t("GATE_SELECTED_HINT", "Parla con il Guardiano dei Supremi!", nil, "AAAAAA")
        syschat("|cffFFD700========================================|r")
...
-- Helper per ottenere nome Supremo dal rank (usa DB o fallback)
function hg_lib.get_supremo_name_by_rank(rank)
    local c, d = mysql_direct_query("SELECT supremo_name FROM srv1_hunabku.hunter_supremo_config WHERE rank_code = '" .. rank .. "' AND enabled = 1 LIMIT 1")
    if c > 0 and d[1] then
        return d[1].supremo_name
    end
    -- Fallback
    local names = { E = "Ombra Minore", D = "Cavaliere Oscuro", C = "Demone delle Ombre", B = "Signore dell'Abisso", A = "Arconte Infernale", S = "Monarca delle Tenebre", N = "Re dei Supremi" }
    return names[rank] or "Supremo"
end
...
function hg_lib.check_overtake(pid, pname, col_name, added_val, label_nice)
    ...
    -- Se abbiamo gi  controllato negli ultimi 60 secondi, skip
    ...
    local q = "(SELECT 'above' as type, player_name, " .. col_name .. " as score FROM srv1_hunabku.hunter_quest_ranking WHERE " .. col_name .. " > " .. new_score .. " ORDER BY " .. col_name .. " ASC LIMIT 1) UNION ALL (SELECT 'below' as type, player_name, " .. col_name .. " as score FROM srv1_hunabku.hunter_quest_ranking WHERE " .. col_name .. " < " .. new_score .. " AND " .. col_name .. " >= " .. my_score .. " AND player_id != " .. pid .. " ORDER BY " .. col_name .. " DESC LIMIT 1)"
...
-- Helper: controlla se un vnum   nella stringa di vnum separati da virgola
        ...
        if party.is_party() then
            ...
            if party_has_emergency == 1 then
                ...
                for i, member_pid in ipairs(pids) do
                    ...
                    -- Supporto multi-vnum: controlla se il vnum ucciso   valido
...
function hg_lib.process_elite_kill(vnum)
    ...
    if pc.getqf("hq_defense_active") ~= 1 and pc.getqf("hq_emerg_active") == 1 then
        -- Usa la funzione multi-vnum per controllare se questo mob   un target
    ...
    -- === FIRST OF DAY BONUS (SOLO MODE) ===
    local solo_activity_type = nil
    if mob_info.type_name == "BOSS" or mob_info.type_name == "BOSS_ELITE" then
        solo_activity_type = "boss"
    elseif mob_info.type_name == "METIN" or mob_info.type_name == "SUPER_METIN" then
        solo_activity_type = "metin"
    elseif mob_info.type_name == "BAULE" then
        solo_activity_type = "chest"
    end
    if solo_activity_type then
        local is_first, fod_mult = hg_lib.check_first_of_day(solo_activity_type)
        if is_first and fod_mult > 1.0 then
            local fod_add = math.floor(base_pts * (fod_mult - 1.0))
            base_pts = base_pts + fod_add
            local fod_percent = math.floor((fod_mult - 1.0) * 100)
            table.insert(modifier_log, {name = "Primo del Giorno!", value = "+" .. fod_percent .. "%", add = fod_add})
            cmdchat("HunterFirstOfDay " .. solo_activity_type .. "|" .. fod_add .. "|" .. fod_percent)
        end
    end
    -- === FINE FIRST OF DAY ===
    ...
    if evt_type == "points" and evt_mult ~= 1.0 then
        ...
        table.insert(modifier_log, {name = "Evento " .. (evt_name or ""), value = "x" .. evt_mult, add = evt_diff})
        -- Registra partecipazione all'evento (se non gi  registrato)
        hg_lib.register_event_participant()
    end
...
function hg_lib.process_normal_kill()
    -- Se c'  Emergency Quest attiva, controlla se scaduta
    if pc.getqf("hq_emerg_active") == 1 then
        local expire = pc.getqf("hq_emerg_expire") or 0
        if get_time() > expire then
            -- SCADUTA! Chiama end_emergency per applicare penalit 
            hg_lib.end_emergency("FAIL")
        end
...
function hg_lib.trigger_random_emergency()
    ...
    local sql_emerg = "SELECT id, name, description, duration_seconds, target_count, target_vnum, reward_points, reward_item_vnum, reward_item_count, difficulty FROM srv1_hunabku.hunter_quest_emergencies WHERE enabled = 1 AND min_level <= " .. lv .. " AND max_level >= " .. lv .. " AND ((min_level >= " .. rank_range.min .. " AND min_level <= " .. rank_range.max .. ") OR (max_level >= " .. rank_range.min .. " AND max_level <= " .. rank_range.max .. ") OR (min_level <= " .. rank_range.min .. " AND max_level >= " .. rank_range.max .. ")) ORDER BY RAND() LIMIT 1"

    local c, d = mysql_direct_query(sql_emerg)

    if c > 0 and d[1] then
        local mission = d[1]
        local reward_pts = tonumber(mission.reward_points) or 0
        -- Penalit  = 50% della ricompensa (configurabile)
        ...
        -- Passa tutti i parametri incluso descrizione, difficolt  e penalit 
...
function hg_lib.open_chest(chest_vnum)
    ...
    -- FIX: Notifica SEMPRE (anche solo player, non solo party)
    ...
    if got_item then
        syschat("|cffFFD700[ITEM]|r Hai ottenuto: " .. item_name_str)
    end
...
function hg_lib.apply_solo_chest_modifiers(player_id, base_glory)
    ...
    local final_glory, modifier_log = hg_lib.calculate_glory_with_modifiers(base_glory, {
        player_id = player_id,
        activity_type = "chest"  -- First of Day bonus per bauli
    })
...
function hg_lib.give_chest_reward()
    ...
    if c > 0 and d[1] then
        ...
        -- NOTA: bonus_points (b) non viene usato qui perche' la gloria
        -- viene gia' assegnata dal flusso principale (on_elite_kill)
        -- con i bonus First of Day, Streak, Rank, ecc.
    end
end
...
-- Helper: Ottiene il progresso per un tipo di achievement
function hg_lib.get_achievement_progress(ach_type)
    -- Type 1: Kill mob
    -- Type 2: Gloria totale
    -- Type 3: Fratture sigillate
    -- Type 4: Bauli aperti
    -- Type 5: Metin distrutti
    -- Type 6: Boss uccisi
    -- Type 7: Missioni giornaliere completate
    -- Type 8: Prove completate
    -- Type 9: Streak massima
    -- Type 10: Rank raggiunto (E=1, D=2, C=3, B=4, A=5, S=6, N=7)
    
    if ach_type == 1 then
        return pc.getqf("hq_total_kills") or 0
    elseif ach_type == 2 then
        return pc.getqf("hq_total_points") or 0
    elseif ach_type == 3 then
        return pc.getqf("hq_total_fractures") or 0
    elseif ach_type == 4 then
        return pc.getqf("hq_total_chests") or 0
    elseif ach_type == 5 then
        return pc.getqf("hq_total_metins") or 0
    elseif ach_type == 6 then
        return pc.getqf("hq_total_bosses") or 0
    elseif ach_type == 7 then
        return pc.getqf("hq_total_missions") or 0
    elseif ach_type == 8 then
        return pc.getqf("hq_total_trials") or 0
    elseif ach_type == 9 then
        return pc.getqf("hq_best_streak") or 0
    elseif ach_type == 10 then
        -- Rank: E=1, D=2, C=3, B=4, A=5, S=6, N=7
        local grade = hg_lib.get_player_rank_grade(pc.get_player_id())
        local rank_values = {["E"]=1, ["D"]=2, ["C"]=3, ["B"]=4, ["A"]=5, ["S"]=6, ["N"]=7}
        return rank_values[grade] or 1
    end
    return 0
end
...
function hg_lib.check_achievements()
    ...
    if d then
        for i=1,table.getn(d) do
            ...
            local prg = hg_lib.get_achievement_progress(at)
...
-- ============================================================
-- SYNC ACHIEVEMENT FLAGS - Sincronizza flag achievement da DB
-- Chiamata al login per ripristinare lo stato degli achievement riscossi
-- ============================================================
function hg_lib.sync_achievement_flags()
    local pid = pc.get_player_id()
    
    -- Query per ottenere tutti gli achievement gi� riscossi dal player
    ...
    -- Se la query fallisce o non ci sono risultati, esci silenziosamente
    if not c or c == 0 or not d then
        return
    end
    ...
    for i = 1, c do
        if d[i] then
            ...
            if aid then
                -- Setta il quest flag per ogni achievement gi� riscosso
...
function hg_lib.spawn_fracture()
    ...
    -- === BLOCCO DUNGEON (CH99): Le fratture NON devono apparire nei dungeon ===
    if is_in_dungeon then
        return  -- Exit silently, no fracture in dungeons
    end
    -- ===========================================================================
    
    -- === FILTRO RANK PLAYER: Spawna solo fratture appropriate al rank ===
    -- Logica: Puoi vedere fratture del TUO rank o MAX 1 rank superiore
    -- Es: E-Rank vede E e D, D-Rank vede E/D/C, C-Rank vede E/D/C/B, etc.
    local player_rank = hg_lib.get_player_rank_grade(pid)
    local rank_order = { E = 1, D = 2, C = 3, B = 4, A = 5, S = 6, N = 7 }
    local player_rank_num = rank_order[player_rank] or 1
    local max_allowed_rank = player_rank_num + 1  -- +1 per la sfida extra
    -- ===========================================================================
    ...
    for i = 1, c do 
        ...
        -- === FILTRO RANK: Estrai la lettera del rank della frattura ===
        local fracture_rank_letter = string.match(rank_label, "^(%a)") or "E"
        local fracture_rank_num = rank_order[fracture_rank_letter] or 1
        
        -- Skip se la frattura e' troppo alta per il player (max +1 rank sopra)
        if fracture_rank_num > max_allowed_rank then
            skip_this = true
        end
        -- ================================================================
...
function hg_lib.open_gate(fname, frank, fcolor, pid)
    ...
    -- Recupera il VNUM della frattura per le missioni giornaliere
    local fracture_vnum = pc.getqf("hq_temp_gate_vnum") or 0
    if fracture_vnum == 0 then
        -- Fallback: prova a ottenere il vnum direttamente dall'NPC
        fracture_vnum = npc.get_race() or 0
    end
    ...
    if force_conquest == 1 then
        ...
        -- Salva i dati per finalize (incluso vnum per missioni)
        if not hunter_defense_data then hunter_defense_data = {} end
        hunter_defense_data[pid] = { rank = frank, fname = fname, color = fcolor, vnum = fracture_vnum }
    ...
    -- Usa coordinate del player (che e' vicino alla frattura quando clicca)
    -- npc.get_x() non esiste in Metin2 Lua standard
    ...
    -- PITY SYSTEM: Applica riduzione mob se attivo
    local pity_applied = false
    total_mobs_req, pity_applied = hg_lib.apply_pity_reduction(total_mobs_req, pid)
    
    if pity_applied then
        syschat("|cff00FF00[PITY SYSTEM]|r Mob ridotti! Solo " .. total_mobs_req .. " nemici da eliminare!")
        hg_lib.party_hunter_speak_color("PITY ATTIVO! Difesa facilitata!", "GREEN")
    end
    ...
    -- Salva se pity e' attivo per questa difesa (per le wave)
    pc.setqf("hq_defense_pity_active", pity_applied and 1 or 0)

    -- Durata difesa in base al rank (E/D/C = 60s, B+ aumenta)
    local duration = hg_lib.get_defense_duration_by_rank(frank)
    
    -- PITY SYSTEM: Applica bonus tempo se attivo
    local pity_time = false
    duration, pity_time = hg_lib.apply_pity_duration_bonus(duration, pid)
    
    if pity_time then
        syschat("|cff00FF00[PITY SYSTEM]|r Tempo extra! Hai " .. duration .. " secondi!")
    end
    ...
    -- Calcola raggio zona difesa (in unita' di gioco)
    -- check_distance e' in "tiles", 1 tile = 100 unita'
    local defense_radius = hg_lib.get_defense_config("check_distance", 10) * 100
    
    -- Converti coordinate locali in coordinate world (x100) per il client Python
    local fx_world = fx * 100
    local fy_world = fy * 100

    -- Invia UI DIFESA FRATTURA dedicata (separata dall'Emergency Quest)
    -- Formato: fracture_name|duration|rank|totalMobs|centerX|centerY|radius
    hg_lib.party_cmdchat("HunterFractureDefenseStart " .. hg_lib.clean_str(defense_title) .. "|" .. duration .. "|" .. frank .. "|" .. total_mobs_req .. "|" .. fx_world .. "|" .. fy_world .. "|" .. defense_radius)
...
function hg_lib.spawn_defense_wave(wave_num, rank_grade)
    local ok, err = pcall(function()
        ...
        -- PITY SYSTEM: Controlla se e' attivo per questa difesa
        local pity_active = (pc.getqf("hq_defense_pity_active") or 0) == 1
        ...
        for i, mob_info in ipairs(wave_data.mobs) do
            ...
            -- PITY SYSTEM: Riduci il numero di mob se pity attivo
            if pity_active then
            ...
            if count > 0 and vnum > 0 then
                local buffer = pity_active and 1 or 2  -- Meno buffer se pity
...
function hg_lib.complete_defense_success()
    ...
    -- Reset flag su TUTTI i membri del party (o solo player se solo)
    if party.is_party() then
        ...
        -- FIX: Reset flag anche per player SOLO (non in party)
        local success_msg = hg_lib.get_text("DEFENSE_SUCCESS", nil, "[HUNTER] DIFESA COMPLETATA CON SUCCESSO!")
        syschat(success_msg)
        
        -- Reset solo le flag di wave e killed, NON il fracture_vid (serve per finalize_gate_opening)
        pc.setqf("hq_defense_x", 0)
        pc.setqf("hq_defense_y", 0)
        pc.setqf("hq_defense_wave", 0)
        pc.setqf("hq_defense_mob_req", 0)
        pc.setqf("hq_defense_mob_killed", 0)
        -- NOTA: NON pulire hunter_defense_data per il solo player qui!
        -- I dati servono per finalize_gate_opening quando clicca il portale
    end
    ...
    -- FIRST OF DAY: Bonus Gloria FISSO per prima DIFESA completata del giorno
    -- Il bonus dipende dal RANK della frattura difesa!
    local is_first_defense, _ = hg_lib.check_first_of_day("defense")
    if is_first_defense then
        -- Ricompensa FISSA in base al rank della frattura difesa
        local defense_fixed_rewards = {
            E = 100,    -- Frattura E-Rank
            D = 200,    -- Frattura D-Rank
            C = 350,    -- Frattura C-Rank
            B = 500,    -- Frattura B-Rank
            A = 750,    -- Frattura A-Rank
            S = 1200,   -- Frattura S-Rank
            N = 2000    -- Frattura N-Rank (massima ricompensa!)
        }
        local defense_bonus = defense_fixed_rewards[frank] or 100
        
        -- Assegna il bonus Gloria fisso
        hg_lib.award_glory_to_player(pid, defense_bonus)
        
        -- Notifica speciale con rank della frattura
        local rank_colors = {E="#94a3b8", D="#00AA00", C="#00CCFF", B="#0066FF", A="#AA00FF", S="#FF6600", N="#FF0000"}
        local rcolor = rank_colors[frank] or "#FFFFFF"
        syschat("|cff00FF00============================================|r")
        syschat("|cffFFD700*** PRIMA DIFESA DEL GIORNO! ***|r")
        syschat("|cff" .. rcolor:sub(2) .. "Frattura " .. frank .. "-Rank conquistata!|r")
        syschat("|cff00FF00Bonus Giornaliero: +|r|cffFFD700" .. defense_bonus .. " Gloria|r")
        syschat("|cff00FF00============================================|r")
        
        -- Effetto client
        cmdchat("HunterFirstOfDay defense|" .. defense_bonus .. "|" .. frank)
        
        -- Party bonus: membri del party ricevono 50% del bonus
        if party.is_party() then
            local pids = {party.get_member_pids()}
            for i, member_pid in ipairs(pids) do
                if member_pid ~= pid then
                    local party_bonus = math.floor(defense_bonus * 0.5)
                    if party_bonus > 0 then
                        hg_lib.award_glory_to_player(member_pid, party_bonus)
                        q.begin_other_pc_block(member_pid)
                        syschat("|cff00FF00[PARTY]|r Prima Difesa del Giorno! +|cffFFD700" .. party_bonus .. " Gloria|r")
                        q.end_other_pc_block()
                    end
                end
            end
    ...
    -- PITY SYSTEM: Resetta streak sconfitte consecutive
    hg_lib.reset_defense_fail_streak(pid)
    ...
    -- FIX EXPLOIT: Avvia timer di 5 minuti per distruggere la frattura se non viene aperta
    -- Questo previene l'exploit del relog/cambio mappa che rendeva la frattura nuovamente disponibile
    if fracture_vid > 0 then
        -- Salva il VID in event_flag per il timer (le qf non sono affidabili dopo relog)
        game.set_event_flag("hq_conq_timeout_vid_" .. pid, fracture_vid)
        cleartimer("hq_conquest_timeout")
        timer("hq_conquest_timeout", 300)  -- 5 minuti = 300 secondi
    end
    ...
    -- Ripristina UI dell'Emergency vera se c'  ancora una attiva
...
function hg_lib.fail_defense(reason)
    ...
    if party.is_party() then
        ...
        -- FIX: Gestione SOLO player (non in party)
        if destroy_on_fail then
            syschat("|cffFF0000[HUNTER]|r DIFESA FALLITA! " .. reason .. " - La Frattura Rank " .. frank .. " e' stata DISTRUTTA!")
        else
            syschat("|cffFFFF00[HUNTER]|r DIFESA FALLITA! " .. reason .. " - La Frattura Rank " .. frank .. " e' ancora li, puoi riprovare!")
        end
        
        -- Reset flag per solo player
        pc.setqf("hq_defense_fracture_vid", 0)
        pc.setqf("hq_defense_x", 0)
        pc.setqf("hq_defense_y", 0)
        pc.setqf("hq_defense_wave", 0)
    ...
    -- FIX: hunter_defense_data gi� pulito nel blocco if/else sopra - rimosso duplicato
    ...
    -- FIX: Usa party_hunter_speak_color per inviare a tutti (party o solo)
    hg_lib.party_hunter_speak_color(msg, "RED")
    ...
    if party.is_party() then
        ...
        for i, member_pid in ipairs(pids) do
            ...
            -- PITY SYSTEM: Incrementa streak per tutti i membri del party
            hg_lib.increment_defense_fail_streak(member_pid)
            q.end_other_pc_block()
        end
    else
        pc.setqf("hq_pending_defense_losses", (pc.getqf("hq_pending_defense_losses") or 0) + 1)
        -- PITY SYSTEM: Incrementa streak sconfitte
        hg_lib.increment_defense_fail_streak(pid)
    end
...
function hg_lib.spawn_gate_mob_and_alert(rank_label, fcolor)
    ...
    local sql_query = "SELECT vnum, name, rank_color FROM srv1_hunabku.hunter_quest_spawns WHERE type_name='" .. sel_type .. "' AND enabled=1 AND rank_tier<=" .. max_tier .. " AND min_level<=" .. lv .. " AND max_level>=" .. lv .. " ORDER BY RAND() LIMIT 1"
    local mc, md = mysql_direct_query(sql_query)
    
    -- Fallback: cerca qualsiasi livello ma sempre rispettando il rank
    if mc == 0 then 
        sql_query = "SELECT vnum, name, rank_color FROM srv1_hunabku.hunter_quest_spawns WHERE type_name='" .. sel_type .. "' AND enabled=1 AND rank_tier<=" .. max_tier .. " ORDER BY RAND() LIMIT 1"
        mc, md = mysql_direct_query(sql_query) 
    end
    
    -- Fallback estremo: se ancora nulla, prendi il mob piu basso di quel tipo
    if mc == 0 then 
        sql_query = "SELECT vnum, name, rank_color FROM srv1_hunabku.hunter_quest_spawns WHERE type_name='" .. sel_type .. "' AND enabled=1 ORDER BY rank_tier ASC LIMIT 1"
        mc, md = mysql_direct_query(sql_query) 
    end
    ...
    if mc > 0 and md[1] then
        ...
        if sel_type == "BAULE" then
            ...
            -- FIX: Invia anche alert UI per il baule spawnnato
            local chest_alert_msg = "BAULE " .. rank_label .. " APPARSO!"
            hg_lib.party_cmdchat("HunterSystemSpeak " .. (fcolor or "GOLD") .. "|" .. hg_lib.clean_str(chest_alert_msg))
            
            -- FIX: Invia notifica specifica per il baule
            local chest_name = mob_name or "Baule del Tesoro"
            local chest_data = hg_lib.clean_str(chest_name) .. "|" .. (fcolor or "GOLD") .. "|" .. rank_label
            hg_lib.party_cmdchat("HunterChestSpawn " .. chest_data)
        else
            -- Gestione Boss/Metin
...
function hg_lib.finalize_gate_opening(vid)
    ...
    -- FIX: Usa hunter_defense_data come fonte primaria se disponibile
    -- (per il player che ha fatto la difesa, i dati sono ancora presenti)
    if hunter_defense_data and hunter_defense_data[pid] then
        -- Usa sempre i dati da hunter_defense_data se disponibili (sono piu' affidabili)
        if hunter_defense_data[pid].rank then
            frank = hunter_defense_data[pid].rank
        end
        if hunter_defense_data[pid].color then
            fcolor = hunter_defense_data[pid].color
        end
    ...
    -- NOTA: La frattura viene gi  rimossa nel when click (prima di chiamare questa funzione)
...
function hg_lib.cleanup_global_tables()
    ...
    if _G.hunter_temp_gate_data then
        ...
        for pid, data in pairs(_G.hunter_temp_gate_data) do
            ...
            -- Rimuovi se pi  vecchio di 1 ora
    ...
    if _G.hunter_mission_throttle then
        for key, timestamp in pairs(_G.hunter_mission_throttle) do
            -- Rimuovi timestamp pi  vecchi di 1 ora
    ...
    if _G.hunter_player_data_cache then
        for pid, timestamp in pairs(_G.hunter_player_data_cache) do
            -- Rimuovi timestamp pi  vecchi di 10 minuti
    ...
    if _G.hunter_kill_tracking then
        for pid, data in pairs(_G.hunter_kill_tracking) do
            -- Rimuovi se pi  vecchio di 2 ore o se il player non   online
    ...
    if _G.hunter_session_glory then
        for pid, session in pairs(_G.hunter_session_glory) do
            -- Rimuovi sessioni pi  vecchie di 2 ore
    ...
    if _G.hunter_spawn_tracking then
        for pid, data in pairs(_G.hunter_spawn_tracking) do
            -- Rimuovi se pi  vecchio di 2 ore
    ...
    -- Se una tabella supera max_entries, rimuovi le entry pi  vecchie
    local global_tables = {
    ...
    for _, table_name in ipairs(global_tables) do
        if _G[table_name] then
            ...
            if count > max_entries then
                -- EMERGENCY: Troppi entry, pulisci i pi  vecchi
                local entries = {}
    ...
    -- NOTA: Non possiamo iterare gli event flags direttamente, quindi questa   una nota
...
-- ============================================================
-- SYNC KARMA/GLORIA - Mantiene Karma = Gloria Totale
-- ============================================================
function hg_lib.sync_karma()
    local pid = pc.get_player_id()
    local c, d = mysql_direct_query("SELECT total_points FROM srv1_hunabku.hunter_quest_ranking WHERE player_id = " .. pid)
    if c > 0 and d[1] then
        local gloria = tonumber(d[1].total_points) or 0
        local target_karma = math.floor(gloria / 10)
        local karma = pc.get_alignment()
        if target_karma ~= karma then
            local diff = target_karma - karma
            pc.change_alignment(diff)
            return gloria - (karma * 10)
        end
    end
    return 0
end

-- Forza karma a un valore specifico (per GM)
-- NOTA: Passa il valore visualizzato, verra' diviso per 10
function hg_lib.set_karma(value)
    local target = math.floor(value / 10)
    local current = pc.get_alignment()
...
function hg_lib.send_player_data(force)
    ...
    -- FIX: Rimosso "AND player_id != r.player_id" - la posizione deve contare SOLO chi ha punteggio SUPERIORE
    local q = "SELECT r.total_points, r.spendable_points, r.daily_points, r.weekly_points, r.total_kills, r.daily_kills, r.weekly_kills, r.total_fractures, r.total_chests, r.total_metins, r.pending_daily_reward, r.pending_weekly_reward, (SELECT COUNT(*) + 1 FROM srv1_hunabku.hunter_quest_ranking WHERE daily_points > r.daily_points) as pos_d, (SELECT COUNT(*) + 1 FROM srv1_hunabku.hunter_quest_ranking WHERE weekly_points > r.weekly_points) as pos_w, r.chests_e, r.chests_d, r.chests_c, r.chests_b, r.chests_a, r.chests_s, r.chests_n, r.boss_kills_easy, r.boss_kills_medium, r.boss_kills_hard, r.boss_kills_elite, r.metin_kills_normal, r.metin_kills_special, r.defense_wins, r.defense_losses, r.elite_kills FROM srv1_hunabku.hunter_quest_ranking r WHERE r.player_id = " .. pid
    ...
    if c > 0 and d[1] then
        ...
        local total_kills = tonumber(d[1].total_kills) or 0
        local total_fractures = tonumber(d[1].total_fractures) or 0
        local total_chests = tonumber(d[1].total_chests) or 0
        local total_metins = tonumber(d[1].total_metins) or 0
        local total_missions = tonumber(d[1].total_missions) or 0
        local total_trials = tonumber(d[1].total_trials) or 0
        
        -- Calcola totale boss (somma dei 4 tipi)
        local total_bosses = (tonumber(d[1].boss_kills_easy) or 0) + 
                             (tonumber(d[1].boss_kills_medium) or 0) + 
                             (tonumber(d[1].boss_kills_hard) or 0) + 
                             (tonumber(d[1].boss_kills_elite) or 0)
        
        -- FIX: Sincronizza TUTTI i qf con il database
        -- Questi sono usati per controlli locali (fratture, achievements, ecc.)
        pc.setqf("hq_total_points", total_pts)
        pc.setqf("hq_total_kills", total_kills)
        pc.setqf("hq_total_fractures", total_fractures)
        pc.setqf("hq_total_chests", total_chests)
        pc.setqf("hq_total_metins", total_metins)
        pc.setqf("hq_total_bosses", total_bosses)
        pc.setqf("hq_total_missions", total_missions)
        pc.setqf("hq_total_trials", total_trials)
        
        -- Sincronizza streak (la migliore � salvata nel qf hq_best_streak)
        ...
        if current_streak > best_streak then
            pc.setqf("hq_best_streak", current_streak)
        end
...
function hg_lib.send_achievements()
    -- Invia SOLO 10 achievement totali: da riscuotere + 1 per categoria in corso
    local MAX_TOTAL = 10
    local all_ach = {}
    
    for atype = 1, 10 do
        local prg = hg_lib.get_achievement_progress(atype)
        local c, d = mysql_direct_query("SELECT id, name, requirement FROM srv1_hunabku.hunter_quest_achievements_config WHERE enabled=1 AND type=" .. atype .. " ORDER BY requirement ASC LIMIT 5")
        
        if c > 0 then
            local found_next = false
            for i = 1, c do
                local aid = tonumber(d[i].id)
                local req = tonumber(d[i].requirement)
                local clm = pc.getqf("hq_ach_clm_" .. aid) or 0
                local unl = (prg >= req) and 1 or 0
                
                -- Da riscuotere = priorita alta (score 100)
                if unl == 1 and clm == 0 then
                    table.insert(all_ach, {id=aid, name=d[i].name, t=atype, req=req, prg=prg, unl=unl, clm=clm, score=100})
                -- Primo in corso per questa categoria
                elseif unl == 0 and not found_next then
                    table.insert(all_ach, {id=aid, name=d[i].name, t=atype, req=req, prg=prg, unl=unl, clm=clm, score=50})
                    found_next = true
                end
            end
        end
    ...
    -- Ordina per score (da riscuotere prima)
    table.sort(all_ach, function(a,b) return a.score > b.score end)
    
    -- Costruisci stringa (max 10)
    local str = ""
    local total = table.getn(all_ach)
    local limit = MAX_TOTAL
    if total < limit then limit = total end
    for i = 1, limit do
        local a = all_ach[i]
        local name = a.name or ""
        if string.len(name) > 20 then name = string.sub(name, 1, 17) .. "..." end
        str = str .. a.id .. "," .. hg_lib.clean_str(name) .. "," .. a.t .. "," .. a.req .. "," .. a.prg .. "," .. a.unl .. "," .. a.clm .. ";"
    end
...
function hg_lib.process_daily_reset()
    -- STEP 1: Applica penalit  per missioni NON completate del giorno precedente
...
-- Applica penalit  gloria per missioni non completate
    ...
    if c > 0 then
        for i = 1, c do
            ...
            if player_id > 0 and penalty > 0 then
                -- Sottrai penalit  SOLO da total_points (non da spendable_points)
                ...
                if penalty > 0 then
                    ...
                    if pc.get_player_id() == player_id then
                        pc.change_alignment(-math.floor(penalty / 10))
                    end
                    q.end_other_pc_block()
                end
    ...
    if c > 0 then
        for i = 1, c do
            ...
            if progress_to_add > 0 then
                ...
                -- Aggiorna solo se c'  effettivamente progresso
...
function hg_lib.update_mission_progress(mission_type, amount, target_vnum)
    ...
    -- Se non   passato abbastanza tempo, esci (il buffer verr  processato dopo)
...
function hg_lib.complete_mission(mission_id)
    ...
    -- FIRST OF DAY: Bonus per prima missione del giorno
    local first_mission_bonus = 0
    local is_first_mission, fod_mult = hg_lib.check_first_of_day("mission")
    if is_first_mission and fod_mult > 1.0 then
        first_mission_bonus = math.floor(reward * (fod_mult - 1.0))
    end
    local total_reward = reward + first_mission_bonus
        
    mysql_direct_query("UPDATE srv1_hunabku.hunter_player_missions SET status='completed', completed_at=NOW() WHERE id=" .. mission_id)
    mysql_direct_query("UPDATE srv1_hunabku.hunter_quest_ranking SET total_points = total_points + " .. total_reward .. ", spendable_points = spendable_points + " .. total_reward .. ", daily_points = daily_points + " .. total_reward .. ", weekly_points = weekly_points + " .. total_reward .. " WHERE player_id=" .. pid)
    -- KARMA: /10
    pc.change_alignment(math.floor(total_reward / 10))
    
    -- FIX: Incrementa contatore missioni completate per achievement
    pc.setqf("hq_total_missions", (pc.getqf("hq_total_missions") or 0) + 1)
    ...
    -- Mostra notifica First of Day se applicabile
    if first_mission_bonus > 0 then
        syschat("|cff00FF00============================================|r")
        syschat("|cffFFD700*** PRIMA MISSIONE DEL GIORNO! ***|r")
        syschat("|cff00FF00  [" .. hg_lib.get_text("MISSION_COMPLETED", nil, "MISSIONE COMPLETATA") .. "]|r |cffFFFFFF" .. name .. "|r")
        syschat("|cffFFD700  +" .. reward .. " Gloria + " .. first_mission_bonus .. " Bonus = " .. total_reward .. " Gloria!|r")
        syschat("|cffAAAAAA(" .. completed_count .. "/3 " .. hg_lib.get_text("COMPLETE", nil, "complete") .. ")|r")
        syschat("|cff00FF00============================================|r")
        cmdchat("HunterFirstOfDay mission|" .. first_mission_bonus .. "|" .. math.floor((fod_mult - 1.0) * 100))
    else
        syschat("|cff00FF00========================================|r")
        syschat("|cff00FF00  [" .. hg_lib.get_text("MISSION_COMPLETED", nil, "MISSIONE COMPLETATA") .. "]|r |cffFFFFFF" .. name .. "|r")
        ...
        if total >= 3 and completed >= 3 then
            ...
            if sc > 0 then
                ...
                if bonus > 0 then
                    ...
                    -- KARMA: /10
                    pc.change_alignment(math.floor(bonus / 10))
...
-- Funzione per controllare se il bonus fratture   ancora attivo
function hg_lib.has_fracture_bonus()
    ...
    if bonus_active == 1 and bonus_day == today_num then
        ...
        -- Resetta il bonus se   il giorno dopo
...
function hg_lib.check_trial_completion_status()
    ...
    if boss_kills > 0 or metin_kills > 0 or chest_opens > 0 or fracture_seals > 0 then
        local flush_q = "UPDATE srv1_hunabku.hunter_player_trials SET boss_kills = boss_kills + " .. boss_kills .. ", metin_kills = metin_kills + " .. metin_kills .. ", chest_opens = chest_opens + " .. chest_opens .. ", fracture_seals = fracture_seals + " .. fracture_seals .. " WHERE player_id = " .. pid .. " AND status = 'in_progress'"
...
function hg_lib.on_boss_kill(boss_vnum)
    ...
    -- DETAILED STATS: Accumula boss per difficolt  (pending flush)
...
function hg_lib.on_fracture_seal()
    ...
    -- Recupera rank e vnum dalla struttura dati della difesa
    local frank = "E"
    local fracture_vnum = 0
    if hunter_defense_data and hunter_defense_data[pid] then
        frank = hunter_defense_data[pid].rank or "E"
        fracture_vnum = hunter_defense_data[pid].vnum or 0
    end
    
    -- Fallback: se vnum non disponibile, prova a recuperarlo dal qf temporaneo
    if fracture_vnum == 0 then
        fracture_vnum = pc.getqf("hq_temp_gate_vnum") or 0
    end
    ...
    -- IMPORTANTE: Passa il vnum della frattura per missioni specifiche (es. Sigillo Frattura Cremisi = 16063)
    hg_lib.update_mission_progress("seal_fracture", 1, fracture_vnum)
    ...
    -- 3. Controlla se la prova   stata completata con questa azione
...
function hg_lib.join_event(event_id)
    ...
    -- KARMA: /10
    pc.change_alignment(math.floor(glory_base / 10))
...
function hg_lib.get_player_achievements(pid)
    -- Prendi configurazione achievements (con reward_glory)
    local cq = "SELECT id, name, type, requirement, reward_vnum, reward_count, COALESCE(reward_glory, 0) as reward_glory FROM srv1_hunabku.hunter_quest_achievements_config WHERE enabled=1 ORDER BY type, requirement"
    ...
    for i = 1, cc do
        ...
        -- Usa la funzione centralizzata per ottenere il progresso
        ...
        table.insert(result, {
            ...
            reward_glory = tonumber(a.reward_glory) or 0
        })
    end
...
function hg_lib.claim_achievement(ach_id)
    ...
    -- Prendi info achievement (incluso reward_glory)
    local q = "SELECT name, type, requirement, reward_vnum, reward_count, COALESCE(reward_glory, 0) as reward_glory FROM srv1_hunabku.hunter_quest_achievements_config WHERE id=" .. ach_id .. " AND enabled=1"
    local c, d = mysql_direct_query(q)
    ...
    -- Mostra ricompense
    if reward_vnum > 0 then
        local item_name = hg_lib.item_name(reward_vnum) or "Oggetto"
        syschat("|cffFFD700[ITEM]|r " .. item_name .. " x" .. reward_count)
    end
    if reward_glory > 0 then
        syschat("|cffFFD700[GLORIA]|r +" .. reward_glory .. " Gloria!")
    end

quest hunter_level_bridge begin
    ...
    state start begin
        ...
        when letter begin
             ...
             -- La gestione � ora affidata in modo sicuro a hg_lib.load_elite_cache()
             ...
             if old_gate_vid > 0 then
                 -- FIX EXPLOIT: NON pulire hq_gate_conq_ se la frattura e' stata conquistata!
                 -- La pulizia della conquista e' gestita dal timer hq_conquest_timeout
                 -- Puliamo solo le flag di DIFESA IN CORSO (lock, time, killed, req)
                 local conq_owner = game.get_event_flag("hq_gate_conq_" .. old_gate_vid) or 0
                 local conq_time = game.get_event_flag("hq_gate_conq_time_" .. old_gate_vid) or 0
                 
                 -- Se la conquista e' del player corrente e ancora nel tempo limite (5 min),
                 -- NON toccare hq_gate_conq_ - lascia che il timer faccia il suo lavoro
                 if conq_owner == pid and conq_time > 0 and (get_time() - conq_time) < 300 then
                     -- Frattura conquistata dal player, timer ancora attivo: NON pulire!
                     -- Pulisci solo le flag della difesa (non della conquista)
                     ...
                     -- Frattura NON conquistata o tempo scaduto: pulisci tutto
...
when chat."/hunter_request_trial_data" begin
        ...
        when login with pc.get_level() >= 5 begin
            local pid = pc.get_player_id()
            local pname = mysql_escape_string(pc.get_name())
            local chk, res = mysql_direct_query("SELECT r.player_id, r.total_points, r.total_kills, r.total_fractures, r.total_chests, r.total_metins, r.boss_kills_easy, r.boss_kills_medium, r.boss_kills_hard, r.boss_kills_elite, IFNULL(r.pending_karma,0) as pending_karma, (SELECT COUNT(*) FROM srv1_hunabku.hunter_player_missions WHERE player_id = " .. pid .. " AND status = 'completed') as total_missions, (SELECT COUNT(*) FROM srv1_hunabku.hunter_player_trials WHERE player_id = " .. pid .. " AND status = 'completed') as total_trials FROM srv1_hunabku.hunter_quest_ranking r WHERE r.player_id = " .. pid)
            if chk == 0 then 
                mysql_direct_query("INSERT INTO srv1_hunabku.hunter_quest_ranking (player_id, player_name, total_points, spendable_points) VALUES (" .. pid .. ", '" .. pname .. "', 0, 0)")
            else 
            ...
            local total_pts = 0
            local total_kills = 0
            local total_fractures = 0
            local total_chests = 0
            local total_metins = 0
            local total_bosses = 0
            local total_missions = 0
            local total_trials = 0
            local pending_karma = 0
            if chk > 0 and res[1] then
                total_pts = tonumber(res[1].total_points) or 0
                total_kills = tonumber(res[1].total_kills) or 0
                total_fractures = tonumber(res[1].total_fractures) or 0
                total_chests = tonumber(res[1].total_chests) or 0
                total_metins = tonumber(res[1].total_metins) or 0
                total_bosses = (tonumber(res[1].boss_kills_easy) or 0) + (tonumber(res[1].boss_kills_medium) or 0) + (tonumber(res[1].boss_kills_hard) or 0) + (tonumber(res[1].boss_kills_elite) or 0)
                total_missions = tonumber(res[1].total_missions) or 0
                total_trials = tonumber(res[1].total_trials) or 0
                pending_karma = tonumber(res[1].pending_karma) or 0
            end
            
            -- KARMA PENDENTE: Aggiunto al total_pts da sincronizzare
            if pending_karma > 0 then
                total_pts = total_pts + pending_karma
                mysql_direct_query("UPDATE srv1_hunabku.hunter_quest_ranking SET pending_karma = 0 WHERE player_id = " .. pid)
            ...
            local target_karma = math.floor(total_pts / 10)
            local current_karma = pc.get_alignment()
            ...
            -- SYNC FIX: Sincronizza i quest flags degli achievement gi� riscossi
            -- Questo risolve il bug "traguardi da ritirare" quando sono gi� stati riscossi
            ...
            -- FIRST OF DAY: Reset flag al login se giorno cambiato
            ...
            -- FIX: Timer eventi spostato qui da letter per garantire esecuzione
            -- Questo assicura che ci sia sempre qualcuno che controlla fine eventi
            cleartimer("hq_event_check_timer")
            loop_timer("hq_event_check_timer", 60)
        ...
        when hq_welcome_msg.timer begin
            -- Usa rank_num gi� calcolato invece di ricalcolarlo dai punti
        ...
        when kill with not npc.is_pc() and pc.get_level() >= 5 begin
            ...
            -- Il level range check � solo per mob normali (spawning fratture)
        ...
        when 16060.click or 16061.click or 16062.click or 16063.click or 16064.click or 16065.click or 16066.click begin
            ...
            -- CONTROLLO LIVELLO MINIMO 75 PER INTERAGIRE CON LE FRATTURE
            local min_level_fracture = 75
            if pc.get_level() < min_level_fracture then
                syschat("|cffFF0000[HUNTER SYSTEM]|r Devi essere almeno livello " .. min_level_fracture .. " per interagire con le fratture!")
                syschat("|cffFFAA00[HUNTER SYSTEM]|r Livello attuale: " .. pc.get_level() .. " | Richiesto: " .. min_level_fracture)
                return
            end
            
            -- FIX: Se la frattura � stata marcata per la distruzione (difesa fallita), rimuovila
            ...
            if owner_pid > 0 then
                ...
                if time_diff > timeout then
                    -- FIX EXPLOIT: TEMPO SCADUTO! La frattura viene DISTRUTTA (non liberata)
                    -- Questo previene l'exploit del relog/cambio mappa
                    ...
                    syschat("|cffFF0000============================================|r")
                    syschat("|cffFF0000[HUNTER SYSTEM] TEMPO SCADUTO!|r")
                    syschat("|cffFF6600[HUNTER SYSTEM] Un Hunter ha perso la sua chance.|r")
                    syschat("|cffFF6600[HUNTER SYSTEM] La frattura e' stata DISTRUTTA.|r")
                    syschat("|cffFFFF00[HUNTER SYSTEM] Cerca un'altra frattura!|r")
                    syschat("|cffFF0000============================================|r")
                    npc.purge()  -- Distruggi la frattura
                    return
                elseif owner_pid == pid then
                    -- E' il proprietario e siamo nel tempo limite -> APRE
                    -- FIX: Pulisci il timer di timeout (non serve piu')
                    cleartimer("hq_conquest_timeout")
                    game.set_event_flag("hq_conq_timeout_vid_" .. pid, 0)
                    ...
                    syschat("|cffFF0000[SISTEMA]|r Verra' distrutta tra " .. left .. " secondi se non viene aperta.")
                    return
                end
            end
        ...
        -- FIX EXPLOIT: Timer per distruggere fratture conquistate ma non aperte entro 5 minuti
        -- Previene l'exploit del relog/cambio mappa che rendeva la frattura nuovamente disponibile
        when hq_conquest_timeout.timer begin
            local pid = pc.get_player_id()
            local fracture_vid = game.get_event_flag("hq_conq_timeout_vid_" .. pid) or 0
            
            if fracture_vid > 0 then
                -- Controlla se il player ha ancora il diritto (potrebbe averla gi� aperta)
                local owner_pid = game.get_event_flag("hq_gate_conq_" .. fracture_vid) or 0
                
                if owner_pid == pid then
                    -- Il player NON ha aperto la frattura in tempo! Distruggi.
                    -- Pulisci tutte le flag associate
                    game.set_event_flag("hq_gate_conq_" .. fracture_vid, 0)
                    game.set_event_flag("hq_gate_conq_time_" .. fracture_vid, 0)
                    game.set_event_flag("hq_gate_data_rank_" .. fracture_vid, 0)
                    game.set_event_flag("hq_gate_data_color_" .. fracture_vid, 0)
                    
                    -- Marca la frattura per la distruzione (verr� rimossa quando qualcuno la tocca)
                    ...
                    syschat("|cffFF0000============================================|r")
                    syschat("|cffFF0000[HUNTER SYSTEM] TEMPO SCADUTO!|r")
                    syschat("|cffFF6600[HUNTER SYSTEM] Non hai aperto la frattura in tempo.|r")
                    syschat("|cffFF6600[HUNTER SYSTEM] La frattura e' stata DISTRUTTA.|r")
                    syschat("|cffFFFF00[HUNTER SYSTEM] Cerca un'altra frattura. Buona caccia!|r")
                    syschat("|cffFF0000============================================|r")
                    
                    hg_lib.hunter_speak_color("HAI PERSO LA TUA CHANCE! FRATTURA DISTRUTTA!", "RED")
                end
                
                -- Pulisci la flag del timer
                game.set_event_flag("hq_conq_timeout_vid_" .. pid, 0)
            end
        end
        ...
        -- RESET UI: Resetta le posizioni delle finestre Hunter alle posizioni default
        -- Utile se un giocatore muove una finestra fuori dallo schermo
        when chat."/hunter_reset_ui" begin
            hg_lib.reset_ui_positions()
        end

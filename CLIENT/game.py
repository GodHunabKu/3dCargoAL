...
# ==============================================================================
	...
	def __HunterSupremoChallenge(self, args):
		...
		try:
			...
			summonerName = parts[0].replace("+", " ") if len(parts) > 0 else "???"
			supremoName = parts[1].replace("+", " ") if len(parts) > 1 else "Supremo"
			vnum = parts[2] if len(parts) > 2 else "0"
			rank = parts[3] if len(parts) > 3 else "E"
			duration = parts[4] if len(parts) > 4 else "600"
			reward = parts[5] if len(parts) > 5 else "500"
			penalty = parts[6] if len(parts) > 6 else "500"
			spawnX = parts[7] if len(parts) > 7 else "0"
			spawnY = parts[8] if len(parts) > 8 else "0"
			maxDistance = parts[9] if len(parts) > 9 else "2000"
	...
	def __HunterSupremoChallengeUpdate(self, args):
		...
		try:
			parts = args.split("|")
			timeLeft = parts[0] if len(parts) > 0 else "0"
			distance = parts[1] if len(parts) > 1 else "0"
